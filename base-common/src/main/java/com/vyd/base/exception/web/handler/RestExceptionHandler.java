 ...
 *     <tr>
 *         <td>errorResolver</td>
 *         <td>{@link DefaultRestErrorResolver1 DefaultRestErrorResolver}</td>
...
public class RestExceptionHandler extends AbstractHandlerExceptionResolver implements InitializingBean {
	...
	protected ModelAndView doResolveException(HttpServletRequest request, HttpServletResponse response, Object handler,	Exception ex) {
		...
		try {
			...
			log.error("Acquiring ModelAndView for Exception[{}] resulted in an exception.", invocationEx);
		}
	...
	private ModelAndView getModelAndView(ServletWebRequest webRequest, Object handler, RestError error) throws Exception {
		...
		return handlerResponseBody(body, webRequest);
	}
	...
	private ModelAndView handlerResponseBody(Object body, ServletWebRequest webRequest) throws ServletException, IOException {
		...
		if (log.isWarnEnabled()) {
			log.warn("Could not find HttpMessageConverter that supports return type [{}] and {}",bodyType,acceptedMediaTypes);
		}
